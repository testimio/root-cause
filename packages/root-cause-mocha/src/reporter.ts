...
export default class RootCauseMochaReporter implements reporters.Base {
    ...
    constructor(public runner: Runner, options: MochaOptions) {
        ...
        Mocha.prototype.reporter.call(thisForMochaReporterFind, options.reporterOptions?.actualReporter ?? '', options.reporterOptions);
        ...
        runner.on('test end', async (test) => {
            ...
            this.testsResultsMovesCompleted.push((async () => {
                ...
                try {
                    ...
                    await fs.move(
                        rootCauseData.testIntermediateResultDir,
                        finalTestResultPath,
                        {}
                    );
        ...
        runner.on('end', async function () {
            ...
            debugLogger(
                `runner end. tests so far: ${reporterInstance.testsWithRootCauseResults.length}`
            );
            ...
            const inputForPrepareRunConclusion = new Map(
                reporterInstance.testsWithRootCauseResults
                    .map((test): {
                        ...
                        const id = utils.testUniqueIdentifierFromStartParams(
                            test.addonData.startTestParams
                        );
