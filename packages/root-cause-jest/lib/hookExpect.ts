...
export type MatcherStartHandler = (matcherName: string, matcherArgs: any[], modifier: typeof matchersModifiers[number] | 'root') => MatcherEndHandler;
...
class ExpectReturnProxyHandler implements ProxyHandler<ExpectReturnType> {
    constructor(private currentModifier: typeof matchersModifiers[number] | 'root', private matcherStartHandler: MatcherStartHandler) {
    ...
    get = (target: ExpectReturnType, matcherNameOrModifier: PropertyKey, receiver: any): any => {
        if (this.currentModifier === 'root') {
            if (isMatcherModifier(matcherNameOrModifier)) {
                return new Proxy(target[matcherNameOrModifier], new ExpectReturnProxyHandler(matcherNameOrModifier, this.matcherStartHandler));
            }
        }
        ...
        return function wrappedFunction(...args: any[]) {
            ...
            try {
                ...
                return returnValue.then(async (resolveValue) => {
