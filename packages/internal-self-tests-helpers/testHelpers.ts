...
export function getMochaTestTimeZeroPrettyFormatPlugin(): jest.SnapshotSerializerPlugin {
    const plugin: Plugin = {
        ...
        serialize(valueToSerialize: string, config, indentation, depth, refs, printer) {
            return printer(valueToSerialize.replace(/\([0-9]+m?s\)/, '(TEST TIME NOISE REMOVED)'), config, indentation, depth, refs, false);
        },
    };
...
export function getCleanAllPathsPrettyFormatPlugin(processCwd: string): jest.SnapshotSerializerPlugin {
    ...
    const plugin: Plugin = {
        ...
        serialize(valueToSerialize: string, config, indentation, depth, refs, printer) {
            return printer(valueToSerialize.split(resolvedProjectRoot).join('noise_removed/'), config, indentation, depth, refs, false);
        },
    };
...
export function getCleanProcessTicksAndRejectionsStackFramePrettyFormatPlugin(): jest.SnapshotSerializerPlugin {
    const plugin: Plugin = {
        ...
        serialize(valueToSerialize: string, config, indentation, depth, refs, printer) {
            const regExp = new RegExp(`${escapeStringRegexp('at processTicksAndRejections (internal/process/task_queues.js')}.+$`, 'g');
