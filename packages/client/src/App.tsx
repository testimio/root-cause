...
const AppComponent = ({ loadTestResult = null, getResourceUrl = defaultExternalResourceUrl, isClickimMode }: AppComponentProps) => {
  ...
  const selectedTabJsx = React.useMemo(() => {
    ...
    if (selectedTab === "screenshots") {
        return <Screenshot showHighlightRect={true} step={selectedStep} className={styles.screenshotContainer} screenshotClassName={styles.image} apiUrl={apiUrl} />;
    }
        ...
        <GetExternalResourceUrlContext.Provider value={getResourceUrl}>
            <MainStoreContext.Provider value={mainStore}>
                <div className={styles.app} onKeyDown={(e) => mainStore.handleKeypress(e.keyCode)} tabIndex={0}>
                    {mainStore.resultsFile.metadata.systemInfo && <TestResultTitlebar isClickimMode={!!isClickimMode} testMetadata={mainStore.resultsFile.metadata} totalTime={mainStore.testTotalTime} />}
                    <StepsSidebar />
                    <div className={styles.layoutSelector}></div>
                    {
                        mainStore.selectedStep && <StepResultTitlebar selectedStep={mainStore.selectedStep} isClickimMode={!!isClickimMode}  selectedTab={selectedTab} selectTab={selectTab} harFileContents={mainStore.harFile} />
                    }
