...
export function useBackgroundHighlight(step: HasRectangle, image: React.RefObject<HTMLImageElement>, container: React.RefObject<HTMLDivElement>, imageStretchBehavior: ImageStretchBehavior) {
    ...
    return useMemo(() => {
        ...
        const ratio = imageStretchBehavior === 'zoom' ? Math.min(widthConstraint, heightConstraint) : Math.max(widthConstraint, heightConstraint);
        ...
        const center = {
            top: -((rect.top + rect.height / 2) / ratio * rect.devicePixelRatio) * backgroundScaleFactor + containerDimensions.height / 2,
            left: -((rect.left + rect.width / 2) / ratio * rect.devicePixelRatio) * backgroundScaleFactor + containerDimensions.width / 2,
        };
