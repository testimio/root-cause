...
describe('Sanity integration test', () => {
    ...
    it('Sanity integration test 1', async () => {
        ...
        const expectedResultsPath = path.resolve(__dirname, 'testsResults', '.root-cause', 'runs', 'mock_invocation_id', testUniqueIdentifierFromStartParams(startTestParams));
        ...
        for (const stepResult of testResults.steps) {
            ...
            if (stepResult.screenshot) {
                ...
                assert.equal(await fs.pathExists(path.resolve(expectedResultsPath, stepResult.screenshot)), true, `${stepResult.screenshot} is missing`);
            }
        }
    });
});
