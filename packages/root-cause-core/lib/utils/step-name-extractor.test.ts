...
describe('Figuring out a step\'s name', () => {
    ...
    it('Uses the step text when there is step text', () => {
        strictEqual(extractStepName(result({ fnName: 'goto', text: 'http://www.example.com' })), 'goto "http://www.example.com"');
        strictEqual(extractStepName(result({ fnName: 'type', text: 'loacker', selector: '#ilan' })), 'type "loacker" on "#ilan"');
    });
