...
export async function logsBeforeAllHook(testContext: TestContext, proxyContext: any, rootPage: RootCausePage) {
    ...
    async function onConsole(message: PuppeteerConsoleMessage) {
        testContext.consoleEntries.push(await puppeteerOrPlaywrightConsoleMessageToOurRepresentation(message, testContext.dateConstructor.now()));
    }

    function onPageError(error: Error) {
        testContext.unhandledExceptions.push(puppeteerPageErrorToOurRepresentation(error, testContext.dateConstructor.now()));
    }
    ...
    function onPageErrorPlaywright(error: Error) {
        testContext.unhandledExceptions.push(playwrightPageErrorToOurRepresentation(error, testContext.dateConstructor.now()));
    }
