...
import { TestimBackendExecutionInputFormat, TestimBackendExecutionFormatSubsetForReporting } from './testim-services-api/interfaces';

export async function persist(runId: string, {
    projectRoot, resultLabel, project, token, jUnitReportPath,
    ...
    if (!credentials.ciToken || !credentials.projectId) {
        console.log('No Root Cause Credentials found, please make sure the TESTIM_PROJECT_ID' +
    ...
    try {
        ...
        spinner.succeed(`Watch your tests in https://app.testim.io/#/project/${credentials.projectId}/runs/suites/${testimFormat.runId}`);
    } catch (e) {
...
async function saveJunitReport({
    jUnitReportPath, testimExecution, projectId,
}: { jUnitReportPath: string; testimExecution: TestimBackendExecutionInputFormat; projectId: string }) {
...
export function generateJunitReport({
    testimExecution, projectId,
}: { testimExecution: TestimBackendExecutionFormatSubsetForReporting; projectId: string }) {
