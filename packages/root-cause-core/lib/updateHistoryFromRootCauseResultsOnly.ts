 ...
 */
export async function updateHistoryFromRootCauseResultsOnly(runId = FALLBACK_RUN_ID, projectRoot = process.cwd(), dateConstructor = Date) {
    ...
    if (!await fs.pathExists(runResultsPath)) {
        throw new Error('None existing Root Cause runs path, make sure you use the same runId & projectRoot as your attach call');
    }
...
function prepareRunConclusionFromOnlyRootCauseResults(runId: string, timestamp: number, data: Map<string, RootCauseRunResultEntry>): RunConclusionFile {
