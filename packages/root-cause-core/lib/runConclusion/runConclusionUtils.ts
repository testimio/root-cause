...
export async function readRunResultsDirToMap(inputDir: string): Promise<Map<string, RootCauseRunResultEntry>> {
    ...
    const resultsDirsList = await pFilter(await fs.readdir(inputDir), async (maybeDirPath) => {
    ...
    const resultsList = await Promise.all(await pMap(resultsDirsList, async (singleResultDirName) => {
