...
export async function puppeteerScreenshot(testContext: TestContext, fnName: string, proxyContext: any, rootPage: RootCausePage, args: any[]) {
    if (!testContext.featuresSettings.screenshots) {
        return;
    }

    const filename = `${testContext.getStepIndex()}.${proxyContext.constructor.name.toLowerCase()}-${fnName}.${testContext.featuresSettings.screenshots.format === 'png' ? 'png' : 'jpg'}`;
    ...
    if (selector) {
        try {
            ...
            const rect = await proxyContext.evaluate((selector: string) => {
                ...
                return {
                    x, y, width, height, top, right, bottom, left, screenWidth, screenHeight, devicePixelRatio,
                };
            }, selector);
