...
describe('Sanity integration test playwright', () => {
    ...
    it('Sanity integration test 1', async () => {
        ...
        const expectedResults = path.resolve(__dirname, 'testsResults', '.root-cause', 'runs', startTestParams.runId, testUniqueIdentifierFromStartParams(startTestParams));

        const testResults: TestResultFile = JSON.parse(await fs.readFile(path.resolve(expectedResults, 'results.json'), 'utf-8'), jsonReduceNoiseReviver);
        ...
        for (const stepResult of testResults.steps) {
            ...
            if (stepResult.screenshot) {
                ...
                assert.equal(await fs.pathExists(path.resolve(expectedResults, stepResult.screenshot)), true, `${stepResult.screenshot} is missing`);
            }
        }
    });
});
