...
export async function launchImpl<T extends AutomationLibrary>(
  ...
  if (options.automationLibrary === 'playwright') {
    // @ts-expect-error
    const playwrightBrowserOptions: PlaywrightBrowserOptions = options.browserOptions
  ...
  const { page, endTest } = await attach(
    { page: realPage, startTestParams, activeFeatures: userSettings.features },
    dateConstructor
  );
  ...
  try {
    ...
    if (!options.noServer) {
      const url = await openServer(TEST_API_PORT, testResultDirFromStartParams(startTestParams));
