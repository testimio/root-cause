...
export class RootCauseExecutionsApi {
  ...
  async createExecution(
    ...
    const conclusion = require(path.join(
      conclusionFolderPath,
      'runConclusion.json'
    )) as RunConclusionFile;
    //TODO(benji)
    ...
    const filePromises = filesToUpload.map((file) =>
      this.testimAssetsApi
        .uploadAsset({
          ...
          pathInsideBucket: `.root-cause/${conclusion.runId}/${path.relative(
            conclusionFolderPath,
            file
          )}`,
          projectId: testimUserMetadata.projectId,
          asset: createReadStream(
            file.endsWith('results.json') ? `${file.substr(0, file.length - 5)}_tmp.json` : file
          ),
          signal,
        })
        .then(() => {
...
async function createTempResultFilesWithResultIds(
  files: string[],
  execution: TestimBackendExecutionInputFormat
) {
  const executionResults = Object.keys(execution.execution).map(
